...
class DiffRow {
  ...
  DiffRow({
    ...
    required this.oldMainItemIndex,
    this.oldSubItemIndex,
  });
}
...
// --- PERFECT SOLUTION: Map Names → Position Queues ---
  void _compareBoqs() {
    ...
    for (int newIdx = 0; newIdx < newItems.length; newIdx++) {
      ...
      _compareMainItem(oldItem, newItem, displayIndex++, matchedOldIdx ?? -1, itemName);
    }
    ...
    for (final entry in oldMainMap.entries) {
      for (final oldIdx in entry.value) {
        ...
        _compareMainItem(oldItem, null, displayIndex++, oldIdx, itemName);
      }
    }
...
// **HELPER: Compare single main item + sub-items**
  void _compareMainItem(Map<String, dynamic>? oldItem, Map<String, dynamic>? newItem, int displayIndex, int oldIdx, String itemName) {
    // Main fields
    ...
    for (int newSubIdx = 0; newSubIdx < newSubs.length; newSubIdx++) {
      ...
      _compareSubItem(oldSub, newSub, displayIndex, subDisplayIndex++, oldIdx, matchedOldSubIdx ?? -1, '$itemName > $subName');
    }
    ...
    for (final entry in oldSubMap.entries) {
      for (final oldSubIdx in entry.value) {
        ...
        _compareSubItem(oldSub, null, displayIndex, subDisplayIndex++, oldIdx, oldSubIdx, '$itemName > $subName');
      }
    }
  }

// **HELPER: Compare single sub-item**
  void _compareSubItem(Map<String, dynamic>? oldSub, Map<String, dynamic>? newSub, int mainIdx, int subIdx, int oldMainIdx, int oldSubIdx, String fullKey) {
    for (var field in _comparisonSubFields) {
      ...
      allDiffRows.add(DiffRow(
        ...
        subItemIndex: subIdx,
        oldMainItemIndex: oldMainIdx,
        oldSubItemIndex: oldSubIdx,
        oldValue: oldVal,
        newValue: newVal,
        isReverted: false,
        isChanged: isChanged,
      ...
      if (subIdx == null) {
        mainGroups.putIfAbsent(
          ...
          () => GroupedDiffItem(
            ...
            mainItemIndex: mainIdx,
            subItemIndex: null,
            oldMainItemIndex: row.oldMainItemIndex,
            oldSubItemIndex: row.oldSubItemIndex,
            fieldDifferences: [],
          ),
        );
  ...
  void _autoRevertNonRemovableItems() {
    ...
    for (var item in groupedItems.where((i) => i.subItemIndex == null)) {
      if (_isItemRemoved(item)) {
        final itemId = _getOldBoqMainId(item.oldMainItemIndex);
    ...
    for (var item in subItems) {
      if (_isItemRemoved(item)) {
        final subId = _getOldBoqSubId(item.oldMainItemIndex, item.oldSubItemIndex!);
        final mainId = _getOldBoqMainId(item.oldMainItemIndex);
    ...
    for (int mainIndex in mainItemsToRevert) {
      final mainItem = groupedItems.firstWhere(
        ...
        orElse: () => GroupedDiffItem(itemKey: '', mainItemIndex: -1, oldMainItemIndex: -1, fieldDifferences: []),
      );
...
// Check if item is REMOVED (all newValue = null)
  ...
  void _revertItemFields(GroupedDiffItem item) {
    final mainItems = _getMainItems(currentBoq);
    final oldMainItems = _getMainItems(widget.oldBoq);
    final oldMainItemIndex = item.oldMainItemIndex;
    final mainItemIndex = item.mainItemIndex;
    final subItemIndex = item.subItemIndex;
    final oldSubItemIndex = item.oldSubItemIndex;
    final status = _getDiffStatus(item);
    debugPrint("SubItemIndex :- $subItemIndex");

    // Handle main item restoration (removed main item)
    if (mainItemIndex >= mainItems.length && oldMainItemIndex > 0 && oldMainItemIndex < oldMainItems.length) {
      final oldItemCopy = Map<String, dynamic>.from(oldMainItems[oldMainItemIndex]);
    ...
    if (subItemIndex == null) {
      ...
      if (mainItemIndex < mainItems.length) {
        final mainItem = mainItems[mainItemIndex] as Map<String, dynamic>;
        final subItems = (mainItem['subItems'] as List<dynamic>?) ?? <dynamic>[];
        final oldSubItems = (oldMainItems.length > oldMainItemIndex && oldMainItemIndex > 0) ? (oldMainItems[oldMainItemIndex] as Map<String, dynamic>)['subItems'] as List<dynamic>? ?? [] : [];

        // If sub-item was removed, create and attach marker directly
        if (subItemIndex >= subItems.length && (oldSubItemIndex != null && oldSubItemIndex > 0 && oldSubItemIndex < oldSubItems.length)) {
          final oldSubItemCopy = Map<String, dynamic>.from(oldSubItems[oldSubItemIndex]);
          // attach marker
        ...
        try {
          if (status == "REMOVED" && !(subItemIndex >= subItems.length && oldSubItemIndex! < oldSubItems.length)) {
            final oldSubItemCopy = Map<String, dynamic>.from(oldSubItems[oldSubItemIndex!]);
            oldSubItemCopy.addAll(_makeRestoredMarker(mainItemIndex, subItemIndex));
...
// Simplified _prepareApiRequest using tags
    ...
    for (final field in fields) {
      final diff = item.fieldDifferences.firstWhere(
        ...
        orElse: () => DiffRow(
          ...
          oldMainItemIndex: item.oldMainItemIndex,
          oldSubItemIndex: item.oldSubItemIndex,
          oldValue: null,
          newValue: null,
          isReverted: true,
          isChanged: false,
  ...
  bool _isNonRemovableDisabled(GroupedDiffItem item) {
    if (item.subItemIndex == null) {
      ...
      final oldMainItems = _getMainItems(widget.oldBoq);
      if (item.oldMainItemIndex < oldMainItems.length) {
        final mainItem = oldMainItems[item.oldMainItemIndex];
    ...
    final id = _getOldBoqSubId(item.oldMainItemIndex, item.oldSubItemIndex!);
    final mainId = _getOldBoqMainId(item.oldMainItemIndex);
